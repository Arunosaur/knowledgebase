...
module.exports = function createSemanticRoutes(deps = {}) {
  ...
  return async function handleSemanticRoute(req, res, pathname, query) {
      ...
      if (graphStore && q) {
        try {
          if (await graphStore.isConnected(config?.bridge?.postgresUrl)) {
            const vectorResults = await graphStore.semanticIntentSearch(q, 10, config.bridge?.ollamaUrl);
          ...
          // Fall through to worker/fallback on error
        }
      }
    ...
    if (pathname === '/semantic/migrate-to-graph' && req.method === 'POST') {
      ...
      (async () => {
        ...
        try {
          const intents = loadLocalIntents();
          console.log(`[GRAPH] Migrating ${intents.length} semantic intents...`);
          for (const intent of intents) {
            try {
              const id = intent.id || `intent-${Date.now()}-${Math.random()}`;
              const intentText = intent.intent || '';
              const keywords = intent.keywords || [];
              const sqlTemplate = intent.sql || '';
              const confidence = parseFloat(intent.confidence) || 0.5;
              const confirmed = intent.confirmed === true;
              const confirmedBy = intent.confirmedBy || '';
              const embedding = await graphStore.generateEmbedding(intentText, config.bridge?.ollamaUrl);
              await graphStore.upsertSemanticIntent(
                id, intentText, keywords, null, sqlTemplate, confidence, confirmed, confirmedBy, embedding
              );
              migrated++;
              if (migrated % 30 === 0) {
                console.log(`[GRAPH] Migrated ${migrated}/${intents.length} intents...`);
              }
            } catch (err) {
              errors.push({ intent: intent.intent, error: err.message.slice(0, 100) });
            }
          }
          console.log(`[GRAPH] ✓ Semantic migration complete: ${migrated} intents`);
        } catch (err) {
          console.error('[GRAPH] Semantic migration failed:', err.message);
