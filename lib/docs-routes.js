...
module.exports = function createDocsRoutes(deps) {
  ...
  return async function handleDocsRoute(req, res, pathname, query) {
    ...
    if (pathname === '/docs/search' && req.method === 'GET') {
      ...
      if (graphStore && q) {
        try {
          if (await graphStore.isConnected(config?.bridge?.postgresUrl)) {
            const vectorResults = await graphStore.semanticDocSearch(q, group || null, lim, config.bridge?.ollamaUrl);
    ...
    if (pathname === '/docs/migrate-to-graph' && req.method === 'POST') {
      // Migrate all docs to PostgreSQL pgvector
