...
module.exports = function createDocsRoutes(deps) {
  ...
  const DOCS_MAX_RESULTS = Number((config.bridge && config.bridge.docsMaxResults) || 10);
  ...
  /**
   * Normalize any extension code variant to canonical form.
   * Handles: "EX01", "EX1", "EX 01", "extension 01", "extension 1",
   *          "ext 01", "ext01", "SDN-215", "SDN215" etc.
   * Returns: { type: 'EX'|'SDN'|null, number: '01'|'215'|null, canonical: 'EX01'|'SDN-215'|null }
   */
  function extractExtensionCode(queryText) {
    const q = String(queryText || '').trim();

    // Pattern 1: Direct code — EX01, EX1, EX 01, EX-01
    const directMatch = q.match(/\bEX[\s\-_]?(\d+)\b/i);
    if (directMatch) {
      const num = directMatch[1].padStart(2, '0');
      return { type: 'EX', number: num, canonical: `EX${num}` };
    }

    // Pattern 2: "extension 01", "extension 1", "ext 01", "ext01"
    const extWordMatch = q.match(/\bext(?:ension)?[\s\-_]?(\d+)\b/i);
    if (extWordMatch) {
      const num = extWordMatch[1].padStart(2, '0');
      return { type: 'EX', number: num, canonical: `EX${num}` };
    }

    // Pattern 3: SDN codes — SDN-215, SDN215, SDN 215
    const sdnMatch = q.match(/\bSDN[\s\-_]?(\d+)\b/i);
    if (sdnMatch) {
      return { type: 'SDN', number: sdnMatch[1], canonical: `SDN-${sdnMatch[1]}` };
    }

    return { type: null, number: null, canonical: null };
  }

  /**
   * Hybrid document search:
   * 1. If query contains an extension code (EX01, "extension 1", etc.)
   *    → search by title ILIKE match (exact code lookup, no semantic needed)
   * 2. Otherwise → pgvector semantic search
   * 3. Fallback → keyword search on JSON files
   */
  async function hybridDocSearch(q, group, limit, postgresUrl, ollamaUrl) {
    const { canonical } = extractExtensionCode(q);
    console.log(`[hybrid] q="${q}" canonical="${canonical}" group="${group}"`);

    // Step 1 — Extension code detected: title-based lookup in pgvector
    if (canonical && graphStore) {
      try {
        const connected = await graphStore.isConnected(postgresUrl);
        console.log(`[hybrid] connected=${connected}`);
        if (connected) {

          // Use the exported pool via a dedicated search function
          const codeResults = await graphStore.searchDocChunksByTitle(
            canonical,
            group || null,
            limit
          );
          console.log(`[hybrid] codeResults.length=${codeResults?.length}`);

          if (codeResults && codeResults.length > 0) {
            console.log(`[docs/search] Extension code "${canonical}" matched ${codeResults.length} chunks by title`);
            return codeResults;
          }

          // If title search found nothing, try broader — maybe "EX01" in chunk text
          const textResults = await graphStore.searchDocChunksByText(
            canonical,
            group || null,
            limit
          );
          if (textResults && textResults.length > 0) {
            console.log(`[docs/search] Extension code "${canonical}" matched ${textResults.length} chunks by text`);
            return textResults;
          }
        }
      } catch (err) {
        console.warn('[docs/search] Extension code search error:', err.message.slice(0, 100));
      }
    }

    // Step 2 — No code or code not found: semantic search
    if (graphStore && q) {
      try {
        if (await graphStore.isConnected(postgresUrl)) {
          const vectorResults = await graphStore.semanticDocSearch(q, group || null, limit, ollamaUrl);
          if (vectorResults && vectorResults.length > 0) {
            return vectorResults;
          }
        }
      } catch (err) {
        console.warn('[docs/search] Semantic search error:', err.message.slice(0, 100));
      }
    }

    // Step 3 — Fallback: keyword search on JSON files
    return docsSearch(q, limit);
  }
  ...
  return async function handleDocsRoute(req, res, pathname, query) {
    ...
    if (pathname === '/docs/search' && req.method === 'GET') {
      ...
      const lim = parseInt(query.limit, 10) || DOCS_MAX_RESULTS;

      const results = await hybridDocSearch(
        q,
        group,
        lim,
        config?.bridge?.postgresUrl,
        config?.bridge?.ollamaUrl
      );
    ...
    if (pathname === '/docs/migrate-to-graph' && req.method === 'POST') {
      ...
      (async () => {
        let migrated = 0, chunks = 0;
        const errors = [];

        try {
          const files = fs.readdirSync(DOCS_INDEX_DIR).filter(f => f.endsWith('.json') && !f.startsWith('jira-'));
          console.log(`[GRAPH] Migrating ${files.length} doc files...`);

          for (const fileName of files) {
            const filePath = path.join(DOCS_INDEX_DIR, fileName);
            try {
              const doc = JSON.parse(fs.readFileSync(filePath, 'utf8'));
              const title = doc.title || doc.fileName || fileName;
              const group = doc.group || fileName.split('-')[0];
              const site = doc.webUrl || doc.site || '';
              const source = doc.source || 'docs-index';
              ...
              if (doc.chunks && Array.isArray(doc.chunks)) {
                for (let idx = 0; idx < doc.chunks.length; idx++) {
                  ...
                  const embedding = await graphStore.generateEmbedding(chunkText, config.bridge?.ollamaUrl);
                  await graphStore.upsertDocChunk(
                    fileName, fileName, title, group, site, source,
                    idx, chunkText, embedding
                  );
                  chunks++;
                }
              }
              ...
              migrated++;
              if (migrated % 50 === 0) {
                console.log(`[GRAPH] Migrated ${migrated}/${files.length} files, ${chunks} chunks...`);
              }
            } catch (err) {
              errors.push({ file: fileName, error: err.message.slice(0, 100) });
            }
          }

          console.log(`[GRAPH] ✓ Migration complete: ${migrated} files, ${chunks} chunks`);
        } catch (err) {
          console.error('[GRAPH] Migration failed:', err.message);
