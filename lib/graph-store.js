...
async function initSchema(postgresUrl) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[GRAPH] AGE extension not available (expected if not in path):', err.message.slice(0, 100));
    }

    // Create AGE graph for dependency analysis
    try {
      await pool.query(`SELECT * FROM ag_catalog.create_graph('wms_dependencies')
        ON CONFLICT DO NOTHING`);
      console.log('[GRAPH] ✓ AGE graph wms_dependencies created');
    } catch (err) {
      if (!err.message.includes('already exists')) {
        console.warn('[GRAPH] AGE graph creation warning:', err.message.slice(0, 100));
      }
    }

    // Create vertex labels
    try {
      await pool.query(`SELECT * FROM ag_catalog.create_vlabel('wms_dependencies', 'OracleObject')
        ON CONFLICT DO NOTHING`);
    ...
    // Create edge labels
    try {
      await pool.query(`SELECT * FROM ag_catalog.create_elabel('wms_dependencies', 'DEPENDS_ON')
        ON CONFLICT DO NOTHING`);
      console.log('[GRAPH] ✓ AGE edge label DEPENDS_ON created');
    } catch (err) {
      console.warn('[GRAPH] AGE edge label creation warning:', err.message.slice(0, 100));
    }

    try {
      await pool.query(`SELECT * FROM ag_catalog.create_elabel('wms_dependencies', 'USED_BY')
        ON CONFLICT DO NOTHING`);
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT * FROM cypher('wms_dependencies', $$
        ...
        RETURN o
      $$, $labels) AS (vertex agtype);
    `, [id, groupId, schemaName, objectName, objectType, status || '']);
...
async function upsertAgeEdge(fromId, toId, dependencyType = 'REFERENCE') {
  ...
  try {
    const result = await pool.query(`
      SELECT * FROM cypher('wms_dependencies', $$
        MATCH (a:OracleObject {id: $1}),
              (b:OracleObject {id: $2})
        MERGE (a)-[r:DEPENDS_ON {type: $3}]->(b)
        RETURN r
      $$, $labels) AS (edge agtype);
    `, [fromId, toId, dependencyType]);
...
async function traverseImpact(objectId, direction = 'both', maxDepth = 3) {
  if (!pool) return { nodes: [], edges: [], queryMs: 0 };

  const startTime = Date.now();

  try {
    // Simplified: return nodes from AGE graph query
    // In production, would filter by direction and depth
    const result = await pool.query(`
      SELECT * FROM cypher('wms_dependencies', $$
        MATCH path = (start:OracleObject {id: $1})-[:DEPENDS_ON*1..$2]-(related)
        RETURN related, length(path) as depth
        LIMIT 200
      $$, $labels) AS (related agtype, depth agtype);
    `, [objectId, maxDepth]);
    ...
    const nodes = result.rows.map((row) => {
      ...
      return {
        id: related?.id || objectId,
        name: related?.name || '?',
        schema: related?.schema || '?',
        type: related?.type || '?',
        depth: row.depth?.value || 0,
      };
    });
    ...
    return { nodes: [], edges: [], queryMs: Date.now() - startTime, truncated: false, error: err.message };
  }
}
