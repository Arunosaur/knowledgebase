CREATE OR REPLACE PACKAGE op_csr_pk IS

  -- Author  : DLBEAL
  -- Created : 2/19/2013 10:38:02 AM
  -- Purpose : CSR Rewrite
...
--------------------------------------------------------------------------------
  ...
  **/
  ...
  PROCEDURE complete_ord_sp(
    ...
    i_mcl_cust             IN  VARCHAR2,
    i_po_num               IN  VARCHAR2,
    i_cancel_ln_parm_list  IN  CLOB,
    i_new_dtl_parm_list    IN  CLOB,
    i_upd_dtl_parm_list    IN  CLOB,
    i_resend_ln_list       IN  CLOB,
    i_user_id              IN  VARCHAR2,
    i_add_on_ord_sw        IN  VARCHAR2 DEFAULT NULL
  );

  PROCEDURE complete_ords_sp(
    i_div                  IN  VARCHAR2,
    i_ord_typ              IN  VARCHAR2,
    i_load_typ             IN  VARCHAR2,
    i_cust_po_parm_list    IN  CLOB,
    i_new_dtl_parm_list    IN  CLOB,
    i_rsn_cd               IN  VARCHAR2,
    i_auth_by              IN  VARCHAR2,
    i_user_id              IN  VARCHAR2,
  ...
  **/
  PROCEDURE ins_ord_sp(
    i_r_ord_hdr   IN  op_csr_pk.g_rt_msg_hdr,
    i_t_ord_dtls  IN  op_csr_pk.g_tt_msg_dtls,
    i_user_id     IN  VARCHAR2,
    i_ord_comnt   IN  VARCHAR2 DEFAULT NULL
  );
...
CREATE OR REPLACE PACKAGE BODY op_csr_pk IS
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
  ...
  */
  FUNCTION ord_num_for_conf_num_fn(
    ...
    RETURN NUMBER IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.ORD_NUM_FOR_CONF_NUM_FN';
  ...
  */
  FUNCTION is_new_ord_fn(
    ...
    RETURN BOOLEAN IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.IS_NEW_ORD_FN';
    lar_parm                logs.tar_parm;
    l_new_sw                VARCHAR2(1);
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  FUNCTION ord_hdr_msg_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module     CONSTANT typ.t_maxfqnm                    := 'OP_CSR_PK.ORD_HDR_MSG_FN';
    lar_parm                logs.tar_parm;
  ...
  BEGIN
    ...
    l_mq_msg_data := rpad_fn(i_r_ord_hdr.div, 2)
                     ...
                     || rpad_fn(CASE i_create_typ
                                  WHEN op_csr_pk.g_c_csr THEN 'CSRWORKBENCH'
                                  WHEN op_csr_pk.g_c_lost_load THEN 'LOST LOAD CREATE'
                                  WHEN op_csr_pk.g_c_copy_order THEN 'COPY ORDER CREATE'
                                END,
                                30
                               )
                     || RPAD('ADD', 13)
  ...
  */
  FUNCTION ord_dtl_msg_fn(
    i_po_num     IN  VARCHAR2,
    i_r_ord_dtl  IN  op_csr_pk.g_rt_msg_dtl
  )
    RETURN VARCHAR2 IS
    l_c_module     CONSTANT typ.t_maxfqnm                 := 'OP_CSR_PK.ORD_DTL_MSG_FN';
    lar_parm             logs.tar_parm;
    l_mq_msg_data        mclane_mq_put.mq_msg_data%TYPE;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  PROCEDURE ord_dtls_sp(
    ...
    o_t_dtl_msgs   OUT     op_csr_pk.g_tt_msgs,
    o_t_ord_lns    OUT     type_ntab
  ) IS
    l_c_module  CONSTANT typ.t_maxfqnm              := 'OP_CSR_PK.ORD_DTLS_SP';
    lar_parm             logs.tar_parm;
    l_ord_lns_sw         VARCHAR2(1);
    l_t_ord_lns          type_ntab;
    l_t_msg_dtls         op_csr_pk.g_tt_msg_dtls;
  ...
  BEGIN
    ...
    o_t_dtl_msgs := op_csr_pk.g_tt_msgs();
  ...
  */
  PROCEDURE parse_cancel_ln_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.PARSE_CANCEL_LN_SP';
    lar_parm             logs.tar_parm;
    l_t_grps             type_stab;
    l_idx                PLS_INTEGER;
  ...
  ||  List is in the following format:
  ||  catlg_num~ord_qty~rsn_cd~auth_by`catlg_num~ord_qty~rsn_cd~auth_by
  ||----------------------------------------------------------------------------
  ||             C H A N G E     L O G
  ||----------------------------------------------------------------------------
  || Date     | USERID  | Changes
  ...
  */
  PROCEDURE parse_ins_ln_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.PARSE_INS_LN_SP';
    lar_parm             logs.tar_parm;
    l_t_grps             type_stab;
    l_idx                PLS_INTEGER;
    l_t_fields           type_stab;
  BEGIN
    ...
    IF l_t_grps IS NOT NULL THEN
      ...
      WHILE l_idx IS NOT NULL LOOP
        ...
        l_t_fields := str.parse_list(l_t_grps(l_idx), op_const_pk.field_delimiter);
        o_t_ins_lns.EXTEND;
        o_t_ins_lns(l_idx).catlg_num := val_at_idx_fn(l_t_fields, 1);
        o_t_ins_lns(l_idx).ord_qty := val_at_idx_fn(l_t_fields, 2);
        o_t_ins_lns(l_idx).rsn_cd := val_at_idx_fn(l_t_fields, 3);
        o_t_ins_lns(l_idx).auth_by := val_at_idx_fn(l_t_fields, 4);
        l_idx := l_t_grps.NEXT(l_idx);
      END LOOP;
    END IF;   -- l_t_grps IS NOT NULL
  ...
  */
  PROCEDURE parse_upd_ln_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.PARSE_UPD_LN_SP';
    lar_parm             logs.tar_parm;
    l_t_grps             type_stab;
    l_idx                PLS_INTEGER;
    l_t_fields           type_stab;
  ...
  */
  PROCEDURE ins_ord_hdr_sp(
    ...
    l_c_module     CONSTANT typ.t_maxfqnm    := 'OP_CSR_PK.INS_ORD_HDR_SP';
    lar_parm              logs.tar_parm;
  ...
  */
  PROCEDURE upd_ord_hdr_sp(
    ...
    i_ord_src   IN  VARCHAR2 DEFAULT NULL
  ) IS
    l_c_module     CONSTANT typ.t_maxfqnm       := 'OP_CSR_PK.UPD_ORD_HDR_SP';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  ...
  BEGIN
    ...
    FETCH l_cv
     INTO l_old_mcl_cust, l_old_po_num, l_old_ord_src;
  ...
  */
  PROCEDURE log_change_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.LOG_CHANGE_SP';
    lar_parm             logs.tar_parm;
  BEGIN
    logs.add_parm(lar_parm, 'DivPart', i_div_part);
    logs.add_parm(lar_parm, 'OrdNum', i_ord_num);
  ...
  */
  PROCEDURE ins_ord_dtl_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm      := 'OP_CSR_PK.INS_ORD_DTL_SP';
    lar_parm             logs.tar_parm;
  ...
  BEGIN
    ...
    IF (    l_t_ins_lns IS NOT NULL
      ...
      FOR i IN l_t_ins_lns.FIRST .. l_t_ins_lns.LAST LOOP
        l_r_ordp120b.lineb := l_ord_ln_cnt + i;
  ...
  */
  PROCEDURE del_new_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.DEL_NEW_SP';
    lar_parm             logs.tar_parm;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
    logs.add_parm(lar_parm, 'DivPart', i_div_part);
  ...
  */
  PROCEDURE cancel_ord_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.CANCEL_ORD_SP';
    lar_parm             logs.tar_parm;
  ...
  */
  PROCEDURE cancel_ord_ln_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.CANCEL_ORD_LN_SP';
    lar_parm             logs.tar_parm;
    l_t_ord_lns          type_ntab;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  PROCEDURE cancel_ord_lns_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.CANCEL_ORD_LNS_SP';
    lar_parm             logs.tar_parm;
  ...
  */
  PROCEDURE upd_ord_dtl_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.UPD_ORD_DTL_SP';
    lar_parm             logs.tar_parm;
  ...
  */
  PROCEDURE save_ord_hdr_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.SAVE_ORD_HDR_SP';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  ...
  BEGIN
    ...
    IF l_mcl_cust IS NOT NULL THEN
      IF (   i_mcl_cust <> l_mcl_cust
        ...
        upd_ord_hdr_sp(i_div_part, i_ord_num, i_po_num, i_mcl_cust, i_ord_src);
  ...
  */
  PROCEDURE save_ord_dtl_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.SAVE_ORD_DTL_SP';
    lar_parm             logs.tar_parm;
...
--------------------------------------------------------------------------------
  ...
  */
  FUNCTION order_typ_list_fn
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.ORDER_TYP_LIST_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  FUNCTION order_detail_status_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.ORDER_DETAIL_STATUS_FN';
    lar_parm                logs.tar_parm;
    l_ord_num               NUMBER;
    l_cv                    SYS_REFCURSOR;
  ...
  || 12/23/11 | rhalpai | Change logic to remove excepion order well.
  || 04/15/13 | dlbeal  | Added 'SHIPPED' as a status option
  ...
  */
  FUNCTION search_order_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module        CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.SEARCH_ORDER_FN';
    lar_parm                   logs.tar_parm;
    -- Moved this from Global constants declarations,
    -- as there is no other procedure using these constants.
    ...
    l_ord_num                  NUMBER;
    l_po_num                   ordp100a.cpoa%TYPE;
    l_wrk_po_num               ordp100a.cpoa%TYPE;
    l_sql_strng                typ.t_maxvc2;
    l_split_typ_strng          VARCHAR2(3000);
    l_conf_strng               VARCHAR2(100);
    l_ord_strng                VARCHAR2(100);
    l_ord_typ_strng            VARCHAR2(100);
    l_store_num_strng          VARCHAR2(500);
    l_crp_cd_strng             VARCHAR2(500);
    l_cbr_cust_strng           VARCHAR2(100);
    l_mcl_cust_strng           VARCHAR2(100);
    l_telxon_strng             VARCHAR2(100);
    l_po_num_strng             VARCHAR2(200);
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
    ...
    l_ord_num := TRIM(i_ord_num);
    l_sql_strng :=
      'SELECT y.Mclane_Cust_No,
                            y.Order_Type,
                            y.Po_No,
                            TO_CHAR(y.Rec_Ts, ''MM/DD/YYYY HH24:MI:SS'') AS Rec_Ts,
                            y.Confirmation_No,
                            y.Order_No,
                            y.Status_Descr,
                            y.Tbl,
                            DATE ''1900-02-28'' + y.Ship_Dt AS Shipdt,
                            y.Maintained_By,
                            y.Test_Order_Type,
                            y.Has_Exception,
                            y.Corp_Cd,
                            y.Header_Status,
                            (SELECT DISTINCT FIRST_VALUE(sd.split_typ)
                                             OVER(PARTITION BY sd.split_typ ORDER BY sd.priorty)
      ...
      || '''
                                         ...
                                         AND (   EXISTS(
                                                   SELECT 1
                                                     FROM split_div_vnd_op3s s,
                                                          strct_item_op3v si, sawp505e e,
                                                          ordp120b b
                                                    WHERE s.split_typ = sd.split_typ
                                                      AND s.div_part = y.div_part
                                                      AND s.div_part = si.div_part
                                                      AND s.cbr_vndr_id = si.cbr_vndr_id
                                                      AND si.item_num = e.iteme
                                                      AND si.uom = e.uome
                                                      AND b.div_part = y.div_part
                                                      AND b.ordnob = y.order_no
                                              ...
                                              OR EXISTS(
                                                   SELECT 1
                                                     FROM split_sta_itm_op1s s, ordp120b b,
                                                          mclp030c mc
                                                    WHERE s.split_typ = sd.split_typ
                                                      AND mc.div_part = y.div_part
                                                      AND mc.custc = y.cust_id
                                                      AND b.div_part = y.div_part
                                                      AND b.ordnob = y.order_no
                            ...
                            y.ord_src
                       FROM (
                             SELECT DECODE(a1.excptn_sw, ''N'', 0, ''Y'', 1) AS Tbl,
--                             SELECT 0 AS Tbl,
                                    a1.div_part,
                                    a1.Stata AS Header_Status,
                                    cx.mccusb AS Mclane_Cust_No,
                                    a1.custa AS cust_id,
                                    a1.Dsorda AS Order_Type,
                                    a1.ipdtsa AS ord_src,
                                    a1.cpoa AS Po_No,
                                    a1.Ord_Rcvd_Ts AS Rec_Ts,
                                    a1.Connba AS Confirmation_No,
                                    a1.Ordnoa AS Order_No,
                                    a1.Shpja AS Ship_Dt,
                                    a1.Mntusa AS Maintained_By,
                                    DECODE(a1.dsorda, ''T'', ''T'') AS Test_Order_Type,
                                    (
                                     SELECT NVL(MAX(''Y''), ''N'')
                                       FROM DUAL
                                      WHERE EXISTS (
                                                    SELECT 1
                                                      FROM ordp120b b
                                                     WHERE b.Ordnob = a1.Ordnoa
                                                       AND b.div_part = a1.div_part
                                                       AND b.excptn_sw = ''Y''
                                                       AND b.Statb IN (''O'', ''S'')
                                                   )
                                    ) AS Has_Exception,
                                    CSR_CUSTOMERS_PK.GET_CORP_CD_MCL_CUST_FN(d.div_id, cx.mccusb) AS Corp_Cd,
                                    OP_CSR_PK.ORDER_DETAIL_STATUS_FN(a1.div_part, a1.Ordnoa, '''') AS Status_Descr
                               FROM div_mstr_di1d d, ordp100a a1, mclp020b cx
                              WHERE d.div_id = '''
      || i_div
      || '''
                                AND a1.div_part = d.div_part
                                AND cx.div_part = a1.div_part
                                AND cx.custb = a1.custa
      ';

    IF i_split_typ IS NOT NULL THEN
      l_split_typ_strng := cnst.newline_char || '                               AND ';
      l_split_typ_strng :=
        l_split_typ_strng
        || '(   EXISTS(
                                         SELECT 1
        ...
        || i_split_typ
        || '''
                                            AND s.div_part = a1.div_part
                                            AND s.ord_num = a1.ordnoa)
                                    OR EXISTS(
                                         SELECT 1
                                           FROM split_div_vnd_op3s s,
                                                strct_item_op3v si, sawp505e e,
        ...
        || '''
                                            ...
                                            AND b.div_part = d.div_part
                                            AND b.ordnob = a1.ordnoa
                                            AND s.cbr_cust = a1.custa
                                            AND s.mcl_item IN(b.orgitb, b.orditb)
                                            AND b.excptn_sw = ''N''
                                            AND b.statb = ''O''
                                            AND b.subrcb = 0)
                                   )';
      l_sql_strng := l_sql_strng || l_split_typ_strng;
    END IF;   -- i_split_typ IS NOT NULL

    IF (    i_conf_num IS NOT NULL
        AND i_exact_conf_num_match_sw = 'N') THEN
      l_conf_strng := cnst.newline_char || '                               AND ';
      l_conf_strng := l_conf_strng || 'a1.connba LIKE ''' || i_conf_num || '%''';
    ELSIF(    i_conf_num IS NOT NULL
          AND i_exact_conf_num_match_sw = 'Y') THEN
      l_conf_strng := l_conf_strng || cnst.newline_char || '                               AND ';
      l_conf_strng := l_conf_strng || 'a1.connba = ''' || i_conf_num || '''';
    END IF;

    IF l_conf_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_conf_strng;
    END IF;

    IF (    i_ord_num IS NOT NULL
        AND i_exact_ord_match_sw = 'N') THEN
      l_ord_strng := cnst.newline_char || '                               AND ';
      l_ord_strng := l_ord_strng || 'a1.ordnoa LIKE ''' || l_ord_num || '%''';
    ELSIF(    i_ord_num IS NOT NULL
          AND i_exact_ord_match_sw = 'Y') THEN
      l_ord_strng := cnst.newline_char || '                               AND ';
      l_ord_strng := l_ord_strng || 'a1.ordnoa = ' || l_ord_num;
    END IF;

    IF l_ord_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_ord_strng;
    END IF;

    IF i_ord_stat = 'UNBILLED' THEN
      l_sql_strng := l_sql_strng || cnst.newline_char || '                               AND ';
      l_sql_strng := l_sql_strng || 'a1.stata IN (''O'', ''S'')';
    ELSIF i_ord_stat = 'BILLED' THEN
      l_sql_strng := l_sql_strng || cnst.newline_char || '                               AND ';
      l_sql_strng := l_sql_strng || 'a1.stata IN (''P'', ''R'')';
    --Shipped separate from Billed
    ELSIF i_ord_stat = 'SHIPPED' THEN
      l_sql_strng := l_sql_strng || cnst.newline_char || '                               AND ';
      l_sql_strng := l_sql_strng || 'a1.stata = ''A'')';
    END IF;

    IF l_ord_typ IS NOT NULL THEN
      l_ord_typ_strng := cnst.newline_char || '                               AND ';
      l_ord_typ_strng := l_ord_typ_strng || 'a1.dsorda = ''' || l_ord_typ || '''';
    END IF;

    IF l_ord_typ_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_ord_typ_strng;
    END IF;

    IF i_store_num IS NOT NULL THEN
      l_store_num_strng := cnst.newline_char
                           || '                               AND cx.storeb LIKE '''
                           || i_store_num
                           || '%''';
    END IF;

    IF l_store_num_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_store_num_strng;
    END IF;

    IF i_crp_cd IS NOT NULL THEN
      l_crp_cd_strng := cnst.newline_char || '                               AND cx.corpb = ' || i_crp_cd;
    END IF;

    IF l_crp_cd_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_crp_cd_strng;
    END IF;

    IF (    i_cust_id IS NOT NULL
        AND LENGTH(TRIM(i_cust_id)) = l_c_cbr_cust_len) THEN
      l_cbr_cust_strng := cnst.newline_char || '                               AND ';
      l_cbr_cust_strng := l_cbr_cust_strng || 'cx.custb = ''' || i_cust_id || '''';
    ELSIF(    i_cust_id IS NOT NULL
          AND LENGTH(TRIM(i_cust_id)) <> l_c_cbr_cust_len) THEN
      l_cbr_cust_strng := l_cbr_cust_strng || cnst.newline_char || '                               AND ';
      l_cbr_cust_strng := l_cbr_cust_strng || 'cx.custb LIKE ''' || i_cust_id || '%''';
    END IF;

    IF l_cbr_cust_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_cbr_cust_strng;
    END IF;

    IF (    i_mcl_cust IS NOT NULL
        AND LENGTH(TRIM(i_mcl_cust)) = l_c_mcl_num_len) THEN
      l_mcl_cust_strng := cnst.newline_char || '                               AND ';
      l_mcl_cust_strng := l_mcl_cust_strng || 'cx.mccusb = ''' || i_mcl_cust || '''';
    ELSIF(    i_mcl_cust IS NOT NULL
          AND LENGTH(TRIM(i_mcl_cust)) <> l_c_mcl_num_len) THEN
      l_mcl_cust_strng := cnst.newline_char || '                               AND ';
      l_mcl_cust_strng := l_mcl_cust_strng || 'cx.mccusb LIKE ''' || i_mcl_cust || '%''';
    END IF;

    IF l_mcl_cust_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_mcl_cust_strng;
    END IF;

    IF i_ser_num IS NOT NULL THEN
      l_telxon_strng := cnst.newline_char || '                               AND ';
      l_telxon_strng := l_telxon_strng || 'a1.telsla LIKE ''' || i_ser_num || '%''';
    END IF;

    IF l_telxon_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_telxon_strng;
    END IF;

    IF i_po_num IS NOT NULL THEN
      l_wrk_po_num := TRANSLATE(UPPER(i_po_num), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 1);
      l_po_num := UPPER(i_po_num);
      l_po_num_strng := cnst.newline_char || '                               AND ';

      IF l_wrk_po_num = l_po_num THEN
        l_po_num_strng := l_po_num_strng || 'a1.cpoa LIKE ''' || i_po_num || '%''';
      ELSE
        l_po_num_strng := l_po_num_strng || 'UPPER(a1.cpoa) LIKE ''' || l_po_num || '%''';
      END IF;
    END IF;

    IF l_po_num_strng IS NOT NULL THEN
      l_sql_strng := l_sql_strng || l_po_num_strng;
    END IF;

    IF i_llr_dt_from IS NOT NULL THEN
      l_sql_strng := l_sql_strng
                     || cnst.newline_char
                     || '                               AND '
                     || 'EXISTS(SELECT 1 FROM load_depart_op1f ld '
                     || 'WHERE ld.load_depart_sid = a1.load_depart_sid '
                     || 'AND ld.div_part = d.div_part '
                     || 'AND ld.llr_dt >= DATE '''
                     || TO_CHAR(TO_DATE(i_llr_dt_from, 'MM/DD/YYYY'), 'YYYY-MM-DD')
                     || ''')';
    END IF;

    IF i_llr_dt_to IS NOT NULL THEN
      l_sql_strng := l_sql_strng
                     || cnst.newline_char
                     || '                               AND '
                     || 'EXISTS(SELECT 1 FROM load_depart_op1f ld '
                     ...
                     || 'AND ld.div_part = d.div_part '
                     || 'AND ld.llr_dt <= DATE '''
                     || TO_CHAR(TO_DATE(i_llr_dt_to, 'MM/DD/YYYY'), 'YYYY-MM-DD')
                     || ''')';
    END IF;

    IF (    i_item IS NOT NULL
        AND i_item_typ = 'CBRITEM'
        AND LENGTH(TRIM(NVL(i_item, ''))) = l_c_cbr_item_len) THEN
      l_sql_strng := l_sql_strng || cnst.newline_char || '                               AND ';
      l_sql_strng :=
        l_sql_strng
        || 'EXISTS (
    ...
    ELSIF(    i_item IS NOT NULL
          AND i_item_typ = 'MCLANEITEM'
          AND LENGTH(TRIM(NVL(i_item, ''))) = l_c_mcl_num_len) THEN
      l_sql_strng := l_sql_strng || cnst.newline_char || '                               AND ';
      l_sql_strng :=
        l_sql_strng
        || 'EXISTS (
    ...
    IF i_incl_hist_sw = 'Y' THEN
      l_sql_strng := l_sql_strng || cnst.newline_char;
      l_sql_strng := l_sql_strng || 'UNION ALL' || cnst.newline_char;
      l_sql_strng :=
        l_sql_strng
        || 'SELECT 2 AS Tbl,
                                              a1.div_part,
                                              ...
                                              CSR_CUSTOMERS_PK.Get_Corp_Cd_Mcl_Cust_Fn(d.div_id, cx.mccusb) AS Corp_Cd,
                                              OP_CSR_PK.Order_Detail_Status_Fn(a1.div_part, a1.Ordnoa, '''') AS Status_Descr
                                         FROM div_mstr_di1d d, ordp900a a1, mclp020b cx
                                        WHERE d.div_id = '''
        || i_div
        || '''
                                  AND a1.div_part = d.div_part
      ...
      IF i_llr_dt_from IS NOT NULL THEN
        l_sql_strng := l_sql_strng
                       ...
                       || '                               AND '
                       || 'a1.ctofda >= '
                       ||(TO_DATE(i_llr_dt_from, 'MM/DD/YYYY') - DATE '1900-02-28');
      END IF;
      ...
      IF i_llr_dt_to IS NOT NULL THEN
        l_sql_strng := l_sql_strng
                       ...
                       || '                               AND '
                       || 'a1.ctofda <= '
                       ||(TO_DATE(i_llr_dt_to, 'MM/DD/YYYY') - DATE '1900-02-28');
      END IF;
    ...
    l_sql_strng := l_sql_strng || ') y';
    l_sql_strng := l_sql_strng || cnst.newline_char;
    l_sql_strng := l_sql_strng || 'ORDER BY y.Status_Descr, y.Order_No DESC';
    logs.dbg(l_sql_strng);
  ...
  */
  FUNCTION hist_detail_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.HIST_DETAIL_FN';
    lar_parm                logs.tar_parm;
    l_div_part              NUMBER;
    l_cv                    SYS_REFCURSOR;
  BEGIN
  ...
  */
  FUNCTION ord_detail_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.ORD_DETAIL_FN';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
    l_cv                 SYS_REFCURSOR;
  ...
  */
  FUNCTION in_hist_as_shipped_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.IN_HIST_AS_SHIPPED_FN';
    lar_parm             logs.tar_parm;
  ...
  */
  FUNCTION in_hist_with_hdr_exception_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.IN_HIST_WITH_HDR_EXCEPTION_FN';
    lar_parm             logs.tar_parm;
    l_found_sw           VARCHAR2(1);
  BEGIN
    logs.add_parm(lar_parm, 'DivPart', i_div_part);
  ...
  */
  FUNCTION hdr_except_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.HDR_EXCEPT_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  ...
  */
  FUNCTION except_order_hdr_list_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.EXCEPT_ORDER_HDR_LIST_FN';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    ...
    IF i_maint_mode_sw = 'Y' THEN
      ...
      op_csr_pk.upd_maint_user_sp(i_div, i_user_id, l_ord_num, '', 'Y');
    END IF;
    ...
    OPEN l_cv
     FOR
       SELECT a.custa,(CASE
              ...
              op_csr_pk.order_detail_status_fn(a.div_part, a.ordnoa) AS ord_stat, lg.rsn, lg.auth_by,
              op_csr_pk.hdr_except_fn(a.div_part, a.ordnoa) AS hdr_except
  ...
  */
  FUNCTION hist_order_hdr_list_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.HIST_ORDER_HDR_LIST_FN';
    lar_parm                logs.tar_parm;
    l_div_part              NUMBER;
    l_cv                    SYS_REFCURSOR;
  BEGIN
    ...
    OPEN l_cv
     FOR
       ...
       SELECT a.custa, c.namec, a.etadta, a.orrtea, a.stopsa, a.ctofda, a.ctofta, a.etadta, a.etatma, d.div_id, NULL,
              ...
              (CASE
                 WHEN op_csr_pk.in_hist_as_shipped_fn(a.div_part, a.ordnoa) = 'Y' THEN 'SHIPPED'
                 WHEN op_csr_pk.in_hist_with_hdr_exception_fn(a.div_part, a.ordnoa) = 'Y' THEN 'HELD'
                 ELSE op_csr_pk.order_detail_status_fn(a.div_part, a.ordnoa, '')
               END
              ) AS ord_stat,
  ...
  */
  FUNCTION order_hdr_list_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.ORDER_HDR_LIST_FN';
    lar_parm                logs.tar_parm;
    l_div_part              NUMBER;
    l_cv                    SYS_REFCURSOR;
  BEGIN
    ...
    IF i_maint_mode_sw = 'Y' THEN
      ...
      op_csr_pk.upd_maint_user_sp(i_div, i_user_id, i_ord_num, '', 'Y');
    END IF;
    ...
    OPEN l_cv
     FOR
       ...
       SELECT a.custa, c.namec, NVL(TRUNC(se.eta_ts), DATE '1900-01-01') - DATE '1900-02-28' AS eta_dt, ld.load_num,
              ...
              op_csr_pk.order_detail_status_fn(a.div_part, a.ordnoa, '') AS ord_stat, lg2.rsn, lg2.auth_by,
  ...
  */
  FUNCTION is_billed_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.IS_BILLED_FN';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
  ...
  */
  FUNCTION reason_cd_list_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.REASON_CD_LIST_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  FUNCTION audit_info_cur_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.AUDIT_INFO_CUR_FN';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_cv                 SYS_REFCURSOR;
  BEGIN
  ...
  */
  FUNCTION list_cnt_fn(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.LIST_CNT_FN';
    lar_parm             logs.tar_parm;
    l_list               typ.t_maxvc2;
  ...
  */
  FUNCTION ord_comment_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm         := 'OP_CSR_PK.ORD_COMMENT_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
    l_ord_comnt          ordp140c.commc%TYPE;
  BEGIN
      ...
      WHEN 'ORD_HDR_MSG_FN' THEN
        ...
        */
        ...
        DECLARE
          ...
          l_r_ord_hdr   op_csr_pk.g_rt_msg_hdr;
          l_ord_comnt   ordp140c.commc%TYPE;
        BEGIN
      ...
      WHEN 'ORD_DTL_MSG_FN' THEN
        ...
        */
        ...
        DECLARE
          ...
          l_r_msg_dtl  op_csr_pk.g_rt_msg_dtl;
        BEGIN
  ...
  */
  FUNCTION query_text_fn(
    ...
    RETURN VARCHAR2 IS
    l_c_module  CONSTANT typ.t_maxfqnm                    := 'OP_CSR_PK.QUERY_TEXT';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  ...
  */
  FUNCTION exception_list_fn
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.EXCEPTION_LIST_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
    ...
    OPEN l_cv
     FOR
       SELECT DISTINCT a.rsncda, a.desca
                  FROM mclp140a a
              ORDER BY a.desca;
  ...
  || 02/26/13 | rhalpai | Convert to use OP1F. PIR11038
  ||----------------------------------------------------------------------------
  */
  FUNCTION ship_date_list_fn(
    i_div              IN  VARCHAR2,
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.SHIP_DATE_LIST_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  */
  FUNCTION query_list_fn(
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.QUERY_LIST_FN';
    lar_parm             logs.tar_parm;
    l_cv                 SYS_REFCURSOR;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
  ...
  || 02/26/13 | rhalpai | Convert to use OP1F. PIR11038
  || 04/23/13 | dlbeal  | Added Manifest category to test bill query
  ||----------------------------------------------------------------------------
  */
  FUNCTION query_dtl_fn(
    i_div              IN  VARCHAR2,
    ...
    RETURN SYS_REFCURSOR IS
    l_c_module     CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.QUERY_DTL_FN';
    lar_parm                logs.tar_parm;
    l_div_part              NUMBER;
    l_cv                    SYS_REFCURSOR;
    l_llr_from              DATE;
  ...
  BEGIN
    ...
    logs.add_parm(lar_parm, 'Div', i_div);
    logs.add_parm(lar_parm, 'OrdTyp', i_ord_typ);
    logs.add_parm(lar_parm, 'LLRFromDt', i_llr_from);
    logs.add_parm(lar_parm, 'LLRToDt', i_llr_to);
    logs.add_parm(lar_parm, 'ShipDt', i_ship_dt);
    logs.add_parm(lar_parm, 'CrpCd', i_crp_cd);
    logs.add_parm(lar_parm, 'Grp', i_grp);
    logs.add_parm(lar_parm, 'ExcptnCd', i_excpt_cd);
    logs.add_parm(lar_parm, 'PONum', i_po_num);
    ...
    l_t_mfsts := str.parse_list(i_mfst_list, l_c_delimiter);
    l_t_custs := str.parse_list(i_cust_list, l_c_delimiter);
    l_t_items := str.parse_list(i_item_list, l_c_delimiter);
    logs.dbg('Open Cursor');
    ...
    IF i_ord_num IS NOT NULL THEN
      ...
      OPEN l_cv
       FOR
         SELECT   LPAD(cx.corpb, 3, '0') AS "Corp", c.retgpc AS "Group", cx.mccusb, c.namec,
                  TO_CHAR(DATE '1900-02-28' + a.shpja, 'YYYY-MM-DD') AS "Ship Date",
                  SUBSTR(a.legrfa, 1, 10) AS "Dist ID", SUBSTR(a.legrfa, 12, 2) AS "SUFFIX", a.ldtypa AS "Load Type",
                  a.cpoa AS "PO Num", b.ordnob AS "Order No", b.orditb AS "MCL ItemNo",
                  (b.manctb || ' ' || mc.descc) AS "Manifest Category", b.hdrtab AS "Retail Amount",
                  b.hdprcb AS "Price", b.ordqtb, NVL(ma.desca, b.ntshpb) "Not Ship Reason", e.shppke AS "Pack",
                  e.sizee AS "Size", e.ctdsce AS "Description",
                  DECODE(b.statb, 'O', 'Open', 'S', 'Suspended', 'I', 'Mainframe', b.statb) AS "Line Status",
                  a.dsorda AS "Order Type", ld.load_num, TO_CHAR(ld.llr_ts, 'YYYY-MM-DD') AS "LLR Date"
  ...
  */
  PROCEDURE upd_for_resend_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.UPD_FOR_RESEND_SP';
    lar_parm             logs.tar_parm;
  ...
  */
  PROCEDURE upd_maint_user_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.UPD_MAINT_USER_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
  BEGIN
  ...
  */
  PROCEDURE send_to_mainframe_sp(
    ...
    l_c_module    CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.SEND_TO_MAINFRAME_SP';
    lar_parm               logs.tar_parm;
    l_div_part             NUMBER;
    l_ord_num              NUMBER;
    l_r_ord_hdr            op_csr_pk.g_rt_msg_hdr;
    ...
    l_t_dtl_msgs           op_csr_pk.g_tt_msgs;
    ...
    PROCEDURE ord_hdr_sp(
      i_div_part   IN      NUMBER,
      i_ord_num    IN      NUMBER,
      o_r_ord_hdr  OUT     op_csr_pk.g_rt_msg_hdr
    ) IS
    BEGIN
      SELECT a.ordnoa,
             d.div_part,
  ...
  BEGIN
    ...
    ELSE
      l_msg_id := 'CSRRESEND';
      l_create_typ := op_csr_pk.g_c_csr;
  ...
  */
  PROCEDURE save_ord_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.SAVE_ORD_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
  ...
  BEGIN
    ...
    ELSE
      ...
      IF (    l_old_ord_src IN('ADK', 'ADC')
        ...
        BEGIN
          l_r_sysp296a.div_part := l_div_part;
          l_r_sysp296a.ordnoa := l_ord_num;
          l_r_sysp296a.linea := 0;
          l_r_sysp296a.usera := i_user_id;
          l_r_sysp296a.tblnma := 'ORDP100A';
          l_r_sysp296a.fldnma := 'IPDTSA';
          l_r_sysp296a.florga := l_old_ord_src;
          l_r_sysp296a.flchga := l_ord_src;
    ...
    save_ord_hdr_sp(l_div_part, l_ord_num, i_conf_num, l_ord_src, i_ord_typ, i_load_typ, i_mcl_cust, i_po_num,
                    i_user_id);
  ...
  */
  PROCEDURE validate_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.VALIDATE_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
    l_is_new_ord         BOOLEAN;
...
--      o_err_msg := COALESCE(NULL, NULL);
  ...
  */
  PROCEDURE suspend_ord_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm := 'OP_CSR_PK.SUSPEND_ORD_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
    l_is_new_ord         BOOLEAN;
  ...
  */
  PROCEDURE complete_ord_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm         := 'OP_CSR_PK.COMPLETE_ORD_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
    l_is_new_ord         BOOLEAN;
  ...
  */
  PROCEDURE complete_ords_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm          := 'OP_CSR_PK.COMPLETE_ORDS_SP';
    lar_parm             logs.tar_parm;
    l_t_grps             type_stab;
    l_idx                PLS_INTEGER;
    l_t_fields           type_stab;
    l_new_dtl_parm_list  typ.t_maxvc2;
    l_cust_id            sysp200c.acnoc%TYPE;
    l_po_num             ordp100a.cpoa%TYPE;
    l_conf_num           ordp100a.connba%TYPE;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
    logs.add_parm(lar_parm, 'Div', i_div);
    logs.add_parm(lar_parm, 'OrdTyp', i_ord_typ);
    logs.add_parm(lar_parm, 'LoadTyp', i_load_typ);
    logs.add_parm(lar_parm, 'CustPoParmList', i_cust_po_parm_list);
    logs.add_parm(lar_parm, 'NewDtlParmList', i_new_dtl_parm_list);
    logs.add_parm(lar_parm, 'RsnCd', i_rsn_cd);
    logs.add_parm(lar_parm, 'AuthBy', i_auth_by);
    logs.add_parm(lar_parm, 'UserId', i_user_id);
    logs.add_parm(lar_parm, 'AddOnOrdSw', i_add_on_ord_sw);
    logs.dbg('ENTRY', lar_parm);
    logs.dbg('Parse Groups of New Detail');
    l_t_grps := str.parse_list(i_new_dtl_parm_list, op_const_pk.grp_delimiter);

    IF l_t_grps IS NOT NULL THEN
      logs.dbg('Building New Detail Parm');
      l_idx := l_t_grps.FIRST;
      WHILE l_idx IS NOT NULL LOOP
        l_new_dtl_parm_list := l_new_dtl_parm_list
                               || l_t_grps(l_idx)
    ...
    logs.dbg('Parse Groups of CustPo');
    l_t_grps := str.parse_list(i_cust_po_parm_list, op_const_pk.grp_delimiter);
    ...
    IF l_t_grps IS NOT NULL THEN
      ...
      WHILE l_idx IS NOT NULL LOOP
        ...
        logs.dbg('Parse Fields');
        l_t_fields := str.parse_list(l_t_grps(l_idx), op_const_pk.field_delimiter);
        l_cust_id := val_at_idx_fn(l_t_fields, 1);
        l_po_num := val_at_idx_fn(l_t_fields, 2);
        l_conf_num := 'C' || next_conf_num_fn;
        logs.dbg('Complete Ord');
        complete_ord_sp(i_div,
                        0,
                        l_conf_num,
                        i_ord_typ,
                        i_load_typ,
  ...
  || 07/16/12 | rhalpai | Change logic to eliminate unused columns. PIR11044
  || 12/23/11 | rhalpai | Change logic to remove excepion order well.
  || 05/16/13 | rhalpai | Add Div in call to INIT_SP. PIR11038
  || 07/04/13 | rhalpai | Change to use OrdTyp to indicate TestSw,NoOrdSw.
  ||                    | PIR11038
  ||----------------------------------------------------------------------------
  */
  PROCEDURE ins_ord_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm      := 'OP_CSR_PK.INS_ORD_SP';
    lar_parm             logs.tar_parm;
    l_div_part           NUMBER;
    l_ord_num            NUMBER;
  ...
  */
  PROCEDURE ins_ord_sp(
    ...
    l_r_ord_hdr          op_csr_pk.g_rt_msg_hdr;
    l_r_ord_dtl          op_csr_pk.g_rt_msg_dtl;
    l_t_ord_dtls         op_csr_pk.g_tt_msg_dtls;
  ...
  */
  PROCEDURE save_query_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm                  := 'OP_CSR_PK.SAVE_QUERY_SP';
    lar_parm             logs.tar_parm;
    l_query_nm           billquery_qu1a.query_name%TYPE;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
    logs.add_parm(lar_parm, 'Div', i_div);
    logs.add_parm(lar_parm, 'QueryNm', i_query_nm);
    logs.add_parm(lar_parm, 'ParmsList', i_parms_list);
    logs.add_parm(lar_parm, 'UserId', i_user_id);
    logs.dbg('ENTRY', lar_parm);
    l_query_nm := UPPER(TRIM(i_query_nm));
    MERGE INTO billquery_qu1a q
      ...
      WHEN MATCHED THEN   --Entry exist - Update the entry for query
        UPDATE
           SET q.query_txt = i_parms_list, q.user_id = i_user_id, q.last_chg_ts = SYSDATE
      WHEN NOT MATCHED THEN   --Entry does NOT exist - Insert an entry for query
        ...
        VALUES(x.div_part, l_query_nm, i_parms_list, i_user_id, SYSDATE);
    COMMIT;
    timer.stopme(l_c_module || env.get_session_id);
    logs.dbg('RUNTIME: ' || timer.elapsed(l_c_module || env.get_session_id) || ' secs.');
  EXCEPTION
  ...
  */
  PROCEDURE rename_query_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm                    := 'OP_CSR_PK.RENAME_QUERY_SP';
    lar_parm             logs.tar_parm;
    l_query_nm           billquery_qu1a.query_name%TYPE;
  ...
  BEGIN
    ...
    logs.add_parm(lar_parm, 'Div', i_div);
    logs.add_parm(lar_parm, 'QueryNm', i_query_nm);
    logs.add_parm(lar_parm, 'NewQueryName', i_new_query_nm);
    logs.add_parm(lar_parm, 'UserId', i_user_id);
    logs.dbg('ENTRY', lar_parm);
    l_query_nm := UPPER(TRIM(i_query_nm));
  ...
  */
  PROCEDURE delete_query_sp(
    ...
    l_c_module  CONSTANT typ.t_maxfqnm                    := 'OP_CSR_PK.DELETE_QUERY_SP';
    lar_parm             logs.tar_parm;
    l_query_nm           billquery_qu1a.query_name%TYPE;
  BEGIN
    timer.startme(l_c_module || env.get_session_id);
