CREATE OR REPLACE PROCEDURE OP_DATA_EBS_DEMANTRA(i_begin_date   DATE DEFAULT NULL,
                                                         i_end_date     DATE DEFAULT NULL
                                                        )

   /*
   ||---------------------------------------------------------------------------
   || OP_EBS_DEMANTRA_DATA
   || EBS - outs from MOQ and cut process
   || OP data extract for EBS DEMANTRA
   || The script is in /local/prodcode/bin> ./OpChgQtyEbs.scr MC
   || Control-m job OpChgQtyEbs_alldays_1600 runs the script daily at 4 PM
   || The script file calls this SP and passes NULL for i_begin_date and i_end_date.
